...
### Fixed
- Switched to native HTTP MCP transport in `plugin.json` to avoid runtime `npx` bootstrap failures
- Improved plugin stability in Claude for Work by removing dependency on `mcp-remote@latest` startup behavior
